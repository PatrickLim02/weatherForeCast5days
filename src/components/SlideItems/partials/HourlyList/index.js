        ...
        <FlexContent>
           {fiveDaysList?.list?.map((item,index) => {
               if(item.dt_txt.includes(convertDate)){
                    return(
                        <Content onClick={()=> directDetailScreen(item)} padding="4%" className="content-container">
                          <TodayLabel fontSize="30px">{moment(item.dt_txt).format("ha")}</TodayLabel>
...
export default connect(mapStateToProps, {setDetails})(HourlyList);
